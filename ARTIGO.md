...
A maioria dos aplicativos de hábitos disponíveis no mercado apresenta o progresso do usuário de forma muito direta: contadores, porcentagens e gráficos de barra. Esses elementos são úteis, mas dificilmente despertam a curiosidade dos usuários, por não ser tão agradável aos olhos. Ao adotar a metáfora do cultivo de plantas, o Semeando Hábitos aproveita algo que já faz parte do senso comum, que é cuidar de uma planta, já que exige atenção constante, e a negligência tem uma consequência visível e imediata, para tornar o monitoramento de hábitos mais intuitivo e engajador.

# 3 Objetivos

### 3.1 Objetivo geral

Desenvolver um site de controle de hábitos que utilize crescimento vegetal como metáfora visual de progresso, incentivando a consistência do usuário por meio de mecânicas de recompensa (crescimento) e consequência (murchamento).

### 3.2 Objetivos específicos

- Permitir o cadastro de hábitos com nome, dias da semana e horário de lembrete;
- Representar cada hábito como uma planta com oito estágios de crescimento, que evolui a cada dia confirmado dentro da agenda escolhida;
- Fazer a planta murchar e zerar a sequência quando um dia programado não é confirmado;
- Calcular e exibir sequência atual, melhor sequência, confirmações totais, consistência dos últimos 30 dias e hábitos ativos;
- Apresentar um mapa de calor das últimas oito semanas e um ranking dos hábitos por sequência;
- Conceder conquistas ao atingir marcos de 7, 30, 66 e 100 dias de sequência;
- Emitir lembretes usando a API de Notificações do navegador;
- Limitar o projeto ao uso de outras linguagens além destas: HTML, CSS e JavaScript. 
- Usar os recursos das pesquisas e limitar o uso de IA para pequenas correções de difícil resolução.
...
- **HTML:** estruturação semântica das seções (cabeçalho, abas de navegação, formulário, cartões de hábito, painéis de estatísticas e conquistas) e uso de um `<template>` para gerar os cartões de planta dinamicamente (não foi tão difícil, já que são as aulas mais recentes do Nilton).
...
- **JavaScript:** toda a lógica de estado, regras de crescimento/murchamento e persistência de dados foi escrita à mão, mas teve algumas coisas que teve que ser Ctrl C, Ctrl V (Nem lembro a quantidade).
